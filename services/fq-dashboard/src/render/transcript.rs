...
/// Add the invocation-detail affordance to the shared edge-error block.
pub fn transcript_error(invocation_id: &str, error: &str) -> String {
    format!(
        r#"{}<p class="muted"><a href="/invocations/{}">invocation detail</a></p>"#,
        super::edge_error("transcript", error),
        esc(invocation_id),
    )
}

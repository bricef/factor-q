...
| `ops` | `factor-q/fq-ops` | the stack's own operations ([ADR-0036](../../docs/adrs/accepted/0036-ops-image-and-scheduler-service.md)): the five `ops/dogfood` scripts, their schedule (`ops.crontab`) under supercronic, and the docker CLI and compose plugin they drive the stack with, on Debian — the one image with a shell and the runtime's socket. No binary: `docker-build` stamps the commit in and `fq-ops --version` reports it. The `ops` service runs it; every operations verb is `docker compose run --rm ops <verb>`. |

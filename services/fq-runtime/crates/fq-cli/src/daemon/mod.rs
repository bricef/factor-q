...
async fn join_with_timeout<E>(name: &str, handle: tokio::task::JoinHandle<Result<(), E>>)
where
    E: std::fmt::Display,
{
    match tokio::time::timeout(std::time::Duration::from_secs(5), handle).await {
        Ok(Ok(Ok(()))) => println!("  {name} stopped cleanly."),
        Ok(Ok(Err(err))) => tracing::error!(error = %err, "{name} exited with error"),
        Ok(Err(err)) => tracing::error!(error = %err, "{name} task panicked"),
        Err(_) => tracing::warn!("{name} did not shut down within 5s"),
    }
}
...
pub(crate) async fn run_daemon(global: &GlobalArgs) -> anyhow::Result<()> {
    ...
    for loaded in registry.iter() {
        start_agent_mcp_servers(&loaded.agent, &mut tools, &mut mcp_manager, true).await;
    }
    ...
    join_with_timeout("projection consumer", projection_handle).await;
    join_with_timeout("coordination consumer", coord_handle).await;
    if let Some(handle) = summary_handle {
        join_with_timeout("summary consumer", handle).await;
    }
    join_with_timeout("heartbeat consumer", hb_consumer_handle).await;
    join_with_timeout("advisory watch", advisory_handle).await;
    join_with_timeout("heartbeat producer", hb_producer_handle).await;
    join_with_timeout("archive-ack consumer", archive_ack_handle).await;
    join_with_timeout("archive retry sweeper", archive_retry_handle).await;

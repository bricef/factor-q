...
**Status:** closed (2026-07-20) — executed through layer 3 (2026-07-12) — `views`, the CLI-as-formatter refactor, the in-daemon tarpc read service, and the `fq-dashboard` binary are all landed on #105; the ADR-0006 read-half note is recorded. The last remainder — deploy-managing the dashboard process — landed 2026-07-14 (`144d542`: deploy.sh manages the dashboard alongside daemon and watcher); dashboard work since continues as ordinary feature commits outside this plan's scope. A web dashboard for *operator* visibility — process state and health metrics — over the running `fq` daemon. Deliberately not a business/product dashboard: there is **no Q, cost-as-leverage, or work-out reporting here** (that belongs with the [M0 close-the-loop](../closed/2026-07-05-m0-close-the-loop.md) metric work, not an ops tool). Grounded in the [2026-07-05 project assessment](../../reviews/2026-07-05-project-assessment.md) (the API layer is "the long pole [with] no active plan"; the observability floor is unbuilt) and lands a first, narrow read-half of [ADR-0006](../../adrs/accepted/0006-registry-first-api.md).

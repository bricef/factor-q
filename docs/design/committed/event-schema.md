...
There is no shim for an older version and none is planned while the runtime is pre-alpha: a build that halts on the stream's history is upgraded or rolled back, and the unacked message is where it resumes. Every consumer built on the shared durable loop halts, not only the projector — the version is a fact about the stream, and every reader of it is behind the same binary. A projection rebuild (the automatic one on a schema bump, or `fq projection rebuild`) replays through this same boundary from the start of stream retention, so a stream still holding older history halts the replay at its first such event; replaying from the first supported version instead is [#648](https://github.com/bricef/factor-q/issues/648).

...
| `maintenance.run_failed` | `notification` | The maintenance consumer ([#344](https://github.com/bricef/factor-q/issues/344)) | `task`, `run_id`, `error` (the task's own message), `duration_ms`. One per failed run, never on a redelivery; a *refused* task raises nothing |

...
**What this demands in practice.** Every piece of runtime-tracked state that an agent might plausibly be asked about has a corresponding read path. Today that is one tool: `self_inspect`, which answers an agent's questions about its own budget, iteration count, tools and sandbox scope. The other read paths this principle asks for — peeking at a sibling agent, tool versions, checkpoint state, handle introspection — do not exist yet, and each absence is a place an agent will infer instead. The principle is the standard; `self_inspect` is how far it has been applied.
...
**None of that substrate exists yet.** There is no graph executor, no fragment library, and none of the four tools named above; the work is deliberately held, not stalled ([#414](https://github.com/bricef/factor-q/issues/414)). So this principle currently does its work as a constraint on what gets built rather than as a description of what runs — the reason there is no bespoke fan-out mechanism in the runtime is that this principle rules one out, and that is a live effect of an unbuilt substrate.
...
**What this demands in practice.** Core capabilities sit behind traits, each carrying the claims and verification the [reducer verification](../../plans/closed/2026-07-05-reducer-verification.md) work established, so a later, better implementation is validated against the same contract. Sophistication (hybrid search, rerankers, a faster engine, smarter compaction) is scheduled as a swap, never as a reason to delay the end-to-end path.

Two of those seams are cut and load-bearing: `EventSink`, with the bus in production and a recording sink in the simulator, and the injected `Clock`, which is what makes the reducer's replay deterministic. The rest are named seams for capabilities that have not been built — the [context policy and strategy](../aspirational/context-management.md), the extractor plugin protocol and vector-engine boundary in the [storage foundation](../../plans/active/2026-06-27-storage-vector-foundation.md), the graph executor (Principle 5) — so their reference implementations (a UTF-8 passthrough extractor, sqlite-vec, a two-stage default compactor) do not exist either. Listing a seam here is a commitment about how the capability will arrive, not a claim that it has.

**Why this compounds — it is what makes fq-work-on-fq tractable.** A module behind a stable, tested seam is exactly what an autonomous agent can safely improve: the interface bounds the blast radius, and the verification net makes the change checkable against a contract the agent cannot silently break. This principle is therefore a precondition for the [M0 self-improvement loop](../../plans/closed/2026-07-05-m0-close-the-loop.md), not merely hygiene — the discipline that keeps a human able to swap a component is the same one that lets an agent do it.

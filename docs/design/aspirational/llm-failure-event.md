...
| `fq events tail` renderer | `fq-cli/src/lib.rs:1545` | **Must** add an arm — exhaustive. Render kind, model and message; render cost inline only when the envelope carries it. |
| `fq events query --type` help | `fq-cli/src/lib.rs:518` | **Should** list `llm_failure`. Query itself is string-keyed and needs no change — the event appears automatically once projected. |

...
| `fq events query --type` help | `fq-cli/src/cli.rs` (`EventCommands::Query`) | **Should** list `llm_failure`. Query itself is string-keyed and needs no change — the event appears automatically once projected. |

...
// address; `github-watcher --probe`, run by the HEALTHCHECK from the same
// environment, asks it and exits 0 on 200. The image is distroless — no
// wget, no curl, no shell — so the binary is its own probe.
//
// Healthy means the NATS connection is up and the poll loop has completed
// an iteration within three intervals. That is "the process is alive and
// attached to the broker", deliberately not "GitHub is reachable": a
// GitHub outage is logged per cycle and is not this process's fault, and
// a probe that failed on it would have the supervisor restarting a
// healthy watcher into the same outage.
//
// adapters/fq-cron/health.go mirrors this file (the two are separate Go
...
const (
	...
	healthBindEnv     = "GHW_HEALTH_BIND"
	defaultHealthBind = "127.0.0.1:9473"
	healthPath        = "/healthz"
...
func serveHealth(ctx context.Context, ln net.Listener, h *Health, log *slog.Logger) error {
	...
	log.Info("health endpoint serving", "addr", "http://"+ln.Addr().String()+healthPath)

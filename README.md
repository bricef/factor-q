...
- [Operating the daemon](docs/guide/operating-the-daemon.md) — stop and redeploy (`fq down`; relaunch after stopping to redeploy), hot-reload (`fq reload`) a running daemon cleanly, and re-derive its projection from the event stream (`fq projection rebuild`)

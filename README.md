...
- [Operating the daemon](docs/guide/operating-the-daemon.md) — [notification and alert vocabulary](docs/guide/operating-the-daemon.md#notifications-and-alerts), stop and redeploy (`fq down`; relaunch after stopping to redeploy), hot-reload (`fq reload`) a running daemon cleanly, and re-derive its projection from the event stream (`fq projection rebuild`)
...
This starts NATS+JetStream in Docker and builds `fq` and `fqd`. Use it through `just fq`:
...
Every `fq` verb answers over the daemon's authenticated edge, so the daemon
has to be up and this client has to be paired with it before any of them
work.
